...
<%
    ...
    Class className = Cms.class;
    Cms entity = (Cms)sh.getJPA().selectByID(className,request);
    ...
    //Cms lastEntity = sh.jpa.selectLast(Cms.class);
            ...
            <div class="page-title" style="margin-top:0;">
                <div class="row">
                    ...
                    for(Language lang : sh.shopSettings.languages){
                        CmsLang entLang = entity!=null?entity.getLang(lang,sh.jpa):null;
                        %>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="col-md-3 control-label">CMS Kategorie</label>
                                <div class="col-md-9">
                                    <select class="form-control select2" name="cms-category-id" >
                                        <%
                                            for(CmsCategory cat : sh.jpa.selectAllByID(CmsCategory.class)){
                                                boolean isSelected = false;
                                                CmsCategoryLang catLang = cat.getLang(sh.shopSettings.defaultLanguage,sh.jpa);
                                                if(entity!=null){
                                                    if(cat.getId()==entity.getCategory().getId())isSelected=true;
                                                }else{
                                                    if(cat.getId()==parentID)isSelected=true;
                                                }
                                                %>
                                                <option value="<%=cat.getId()%>" <%=isSelected?"selected":""%>><%=catLang.getName()%></option>
                                                <%
                                            }
                                        %>
                                    </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            for(Language lang : sh.shopSettings.languages){
                                ...
                                %>
                                <div class="form-group lang-group-<%=lang.getIsoCode()%> lang-group <%=lang.getIsoCode().equals(sh.shopSettings.defaultLanguage.getIsoCode()) ?"shown":""%>">
                                    <label class="col-md-3 control-label">Obsah(<%=lang.getIsoCode()%>)</label>
                                    <div class="col-md-9">
                                        <textarea name="content-<%=lang.getIsoCode()%>" class="ckeditor-sm-init form-control"><%=entLang!=null?entLang.getContent():""%></textarea>
                                    </div>
                                </div>
                                <%
                            }
